...
#
"""Helper functions to access Google SecOps API using OAuth 2.0.
...
def initialize_http_session(
    google_secops_api_credentials: str, scopes: List[str] | None = None
) -> requests.AuthorizedSession:
    """Initializes an authorized HTTP session, based on the given credentials.

    Args:
        google_secops_api_credentials: The private service account info in Google format.
        scopes: A list of OAuth scopes (https://oauth.net/2/scope/) that are associated with the endpoints to be
          accessed. The default is the Google SecOps API scope.

    Returns:
        HTTP session object to send authorized requests and receive responses.
    """
    credentials = service_account.Credentials.from_service_account_info(
        info=google_secops_api_credentials, scopes=scopes or AUTHORIZATION_SCOPES
    )
